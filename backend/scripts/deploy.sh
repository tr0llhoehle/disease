...
RUN_CMD="docker run -d -p ${PORT}:${PORT} -p ${PORT_SSL}:${PORT_SSL} -v /home/patrick/disease:/var/db -v /home/patrick/disease.tr0llhoehle.de:/var/ssl --env APP_DB=${APP_DB} --env APP_CERT=${APP_CERT} --env APP_KEY=${APP_KEY} --restart=on-failure ${IMAGE} npm run start"
